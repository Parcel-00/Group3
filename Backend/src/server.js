...
async function initializeProcessor() {
  const manifestDir = path.join(
    __dirname,
    '../../ShipmentManifestTextFiles'
  );
  const uniqueIdDir = path.join(
    __dirname,
    '../../ShipmentUniqueIdentifiers'
  );
 ...
 */
app.post('/api/shipments/bulk', upload.array('images', 10), async (req, res) => {
  try {
    if (!processor) {
      return res.status(503).json({ error: 'Processor not ready' });
    }
...
app.get('/api/shipments/json/:filename', async (req, res) => {
  try {
    ...
    if (!path.resolve(jsonPath).startsWith(path.resolve(__dirname, '../shipments'))) {
